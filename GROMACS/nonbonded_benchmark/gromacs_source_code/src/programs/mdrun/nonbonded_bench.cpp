...
int gmx_nonbonded_bench(int argc, char *argv[])
{
    std::vector<const char *>        desc = {
        ...
        "within a compute node. In this benchmarks there is no interaction",
        "between threads, apart from starting and closing a single OpenMP",
        "parallel region per iteration. Additionally thread interact",
        "through sharing and evicting data from shared cashes.",
        "The number of threads to use is set with the [TT]-nt[tt] option.",
        "Thread affinity is important, especially with SMT and shared",
        "cashes. Affinities can be set through the OpenMP library using",
        ...
        "correspond to actual clock cycles. For each kernel the total",
        "number of cycles is reported, the cycles per iteration, as well",
        "as the total and useful number of pair interactions per cycle.",
        "Because a cluster pair list is ued instead atom pair list,",
        ...
        "Up till now, we only found tabulated Ewald correction to be useful",
        ...
        "default. The only exception are kernels without SIMD, which only",
        "support tables."
